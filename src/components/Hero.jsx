...
import { WHATSAPP_LINK, PROFISSIONAL_CRF, ENABLE_ONLINE_BOOKING, BOOKING_FEE_BRL } from '../constants'

const FALLBACK = 'https://placehold.co/480x640/4a3280/e8c47a?text=Ingrid+Melo'

export function Hero() {
  const ctaHref = ENABLE_ONLINE_BOOKING ? '#agendamento' : WHATSAPP_LINK
  const ctaTarget = ENABLE_ONLINE_BOOKING ? undefined : '_blank'
      ...
      <div className="section-wrapper relative grid gap-14 md:grid-cols-[1.15fr,1fr] items-center">
        <div>
          ...
          <h1 className="font-playfair text-5xl md:text-[3.6rem] font-bold text-brand-500 leading-[1.15] mb-5 animate-fade-up"
            ...
            Sua pele mais jovem
            <span className="block font-normal italic" style={{ color: '#c9956c' }}> começa com um</span>
            <span className="block">protocolo único.</span>
          </h1>

          <div className="divider-gold" />

          <p className="text-brand-400 text-lg leading-relaxed mb-6 max-w-md animate-fade-up"
          ...
          <div className="flex items-center gap-3 mb-8 animate-fade-up" style={{ animationDelay: '0.25s' }}>
            ...
            <p className="text-sm text-brand-400">
              <span className="font-semibold text-brand-500">+500 pacientes</span> já transformaram sua pele
            </p>
          </div>
          ...
          <div className="mt-8 flex flex-wrap gap-4 animate-fade-up" style={{ animationDelay: '0.4s' }}>
            {[PROFISSIONAL_CRF, 'ANVISA · 100% certificado', 'Ambiente esterilizado'].map((s) => (
              <span key={s} className="flex items-center gap-1.5 text-xs text-brand-400">
                <svg className="w-4 h-4" style={{ color: '#c9956c' }} fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="flex justify-center animate-fade-in" style={{ animationDelay: '0.35s' }}>
          <div className="relative">
            ...
            <div className="relative rounded-[2rem] overflow-hidden shadow-2xl w-72 md:w-80 aspect-[3/4] bg-brand-100">
              <img src={heroPic} alt="Ingrid Melo — Farmacêutica Especialista em Gerenciamento de Pele em Joinville SC"
                   className="w-full h-full object-cover object-top"
                   onError={(e) => { e.currentTarget.src = FALLBACK }} />
              <div className="absolute bottom-0 left-0 right-0 h-28"
                   style={{ background: 'linear-gradient(to top, rgba(46,29,84,0.55), transparent)' }} />
            </div>
            <div className="absolute -bottom-5 -left-5 rounded-2xl shadow-xl px-4 py-3 border" style={{ backgroundColor: '#2e1d54', borderColor: '#4a3280' }}>
              <p className="text-[10px] font-semibold uppercase tracking-wider" style={{ color: '#e8c47a' }}>Farmacêutica · Gerenciamento de Pele</p>
              <p className="font-playfair text-sm font-bold text-white">{PROFISSIONAL_CRF}</p>
            </div>
            <div className="absolute -top-3 -right-3 rounded-2xl shadow-xl px-3 py-2 border text-center" style={{ backgroundColor: '#2e1d54', borderColor: '#4a3280' }}>
              <p className="font-playfair text-lg font-bold" style={{ color: '#e8c47a' }}>4.9</p>
              <div className="flex gap-0.5 justify-center">
                {[...Array(5)].map((_, i) => (
                  <svg key={i} className="w-3 h-3" style={{ color: '#c9956c' }} viewBox="0 0 20 20" fill="currentColor">
                    <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                  </svg>
                ))}
              </div>
              <p className="text-[9px] mt-0.5" style={{ color: '#9b87f5' }}>Google</p>
            </div>
          </div>
        </div>
      </div>
